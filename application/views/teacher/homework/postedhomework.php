			...
			<div class="mycardbody">
				...
				foreach ($data as $v) {
					...
					$temp=explode("=", $gd);
						...
						$str.="</table>";
						if($gd==null){$str="";}
					echo "<tr><td>$v->subject</td><td>$v->chapter</td><td style='padding:0px 0'>$str</td></td><td><a href='".base_url()."teacher/edithomework/$v->id'><i class='fa fa-pencil'></i></a></td><td><i class='fa fa-close' onclick=\"showDelete($v->id,this)\"></i></td><td>$v->datetime</td></tr>";
					$arr[]=$v->id;
				}
				 ?>
				</table>
...
function deleteData(id,ele){
	$.post({url:baseurl+'teacher/deletehomework',data:{'id':id},success:function(data){
...
function deleteEntry(id,ele){
	$.post({url:baseurl+'teacher/deletehomeworkentry',data:{'id':id},success:function(data){
		if(JSON.parse(data).status){ ele.parentElement.parentElement.style.display='none';}
		else{
			openModal({title:'Status',content:JSON.parse(data).message});
		}
...
function showDelete(delid,ele){
 ...
 function updateData(id,column,value){
 	$.post({url:baseurl+'teacher/updatehomeworkentry',data:{id:id,column:column,data:value},success:function(data){
