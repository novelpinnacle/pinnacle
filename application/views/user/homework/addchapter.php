...
<div id="content">
    <div class="mycard">
        ...
        <div class="mycardbody">
            <form id="form" action="<?=base_url()?>user/savechapter" method="post">
                <div class="row">
                    <div class="col-sm-3 wp pl">
                        <div class="form-group">
                            <label>Select Batch</label>
                            <select id="batch" name="bid" onchange="loadSubjects(this.value,subject)"
                                class="w3-input w3-border batch">
...
function addMore() {
    ...
    ele.innerHTML = '<div class="row">' +
        ...
        '<button type="button" class="btn btn-default btn-sm" onclick="addMore()">' +
        '<i class="fa fa-plus-square"></i> Chapters' +
        '</button> <button type="button" class="btn btn-default btn-sm" onclick="document.getElementById(' + "'" + id +
        "'" + ').remove()">' +
...
function loadSubjects(bid, ele) {
    ...
    $.post({
        url: baseurl + "user/getsubjectsbybatchid",
        data: {
            'bid': bid
        },
...
function deleteChapter(id, ele) {
    $("#loading").show();
    $.post({
        url: baseurl + 'user/deletechapter',
        data: {
            'id': id
        },
        success: function(data) {
