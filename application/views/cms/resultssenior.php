	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-sm-3">
				<div class="mycard">
					<div class="mycardheader b-primary">
						<h4 class="mycardtitle">Add New Senior Result</h4>
						<?php 
							$checked=0;			
							if($status==1){
								$checked="checked";
						...
						?>
							<label class="switch">
							<input type="checkbox" <?=$checked?> onchange="toggle('resultssenior')">
							<span class="slider round"></span>
							</label>
					</div>
					<div class="mycardbody">
						<form id="form" action="<?=base_url()?>cms/uploadresultssenior" method="post">
			...
			<div class="col-sm-9 slideshow-col">
				<div class="mycard">
					<div class="mycardheader b-primary">
						<h4 class="mycardtitle">Active Senior Toppers</h4>
					</div>
					<div class="mycardbody">
					<div class='table-wrapper' id="loadtable">
					<table class='sc-table'>
						<tr><th>Image</th><th>Name</th><th>Class</th><th>Marks</th><th>Edit</th><th>Delete</th></tr>
						<?php foreach ($resultssenior as $v) {
							echo "<tr><td style='width:100px;padding:2px'><img src='".base_url()."$v->image' class='thumbnail-img'></td><td>$v->name</td><td>$v->classdetails</td><td>$v->marks</td><td><a href='".base_url()."cms/editresultssenior/$v->id'><i class='fa fa-pencil'></i></a></td><td><i class='fa fa-close' onclick='deleteResultsSenior($v->id,this)'></i></td></tr>";
						} ?>
					</table>
					</div>
					</div>
...
function deleteResultsSenior(tid,ele){
$.post({url:baseurl+'cms/deleteresultssenior',data:{'tid':tid},success:function(data){
ele.parentElement.parentElement.style.display='none';
}});
}
function refreshData(){
$.get({url:baseurl+'cms/refreshresultssenior',success:function(data){loadtable.innerHTML=data;}});
}
</script>
